...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // Per-side longhands only. `borderColor` / `borderWidth` are shorthands too:
    // pairing them with `borderLeftColor` makes React warn (and mis-style) when
    // `focused` flips on rerender, e.g. j/k focus or a severity filter change.
    borderStyle: "solid",
