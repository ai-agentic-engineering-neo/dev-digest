 ...
 *  - `skills`    — the whole ordered set, each with its own enabled flag (current).
 *  - `skill_ids` — legacy "set the whole set" form; each link defaults enabled.
 *  - `skill_id`  — legacy "link one" form (+ optional `order`/`enabled`).
 */
