import { describe, it, expect } from "vitest";
import type { FindingRecord } from "@devdigest/shared";
import { visibleFindings } from "./helpers";

function finding(over: Partial<FindingRecord>): FindingRecord {
  return {
    id: "f1",
    severity: "WARNING",
    category: "security",
    title: "t",
    file: "src/a.ts",
    start_line: 1,
    end_line: 1,
    rationale: "r",
    suggestion: null,
    confidence: 0.9,
    kind: "finding",
    trifecta_components: null,
    evidence: null,
    review_id: "r1",
    accepted_at: null,
    dismissed_at: null,
    ...over,
  };
}

const FINDINGS: FindingRecord[] = [
  finding({ id: "s1", severity: "SUGGESTION" }),
  finding({ id: "c1", severity: "CRITICAL" }),
  finding({ id: "w1", severity: "WARNING" }),
  finding({ id: "c2", severity: "CRITICAL", confidence: 0.3 }),
];

describe("visibleFindings", () => {
  it("no filter keeps everything, sorted by severity", () => {
    expect(visibleFindings(FINDINGS, false).map((f) => f.severity)).toEqual([
      "CRITICAL",
      "CRITICAL",
      "WARNING",
      "SUGGESTION",
    ]);
  });

  it("keeps only the selected severity", () => {
    expect(visibleFindings(FINDINGS, false, "CRITICAL").map((f) => f.id)).toEqual(["c1", "c2"]);
  });

  it("composes with hide-low-confidence", () => {
    expect(visibleFindings(FINDINGS, true, "CRITICAL").map((f) => f.id)).toEqual(["c1"]);
  });

  it("keeps DISMISSED findings in the list — only the counters exclude them", () => {
    const dismissed = finding({ id: "d1", severity: "CRITICAL", dismissed_at: "2026-09-17T00:00:00Z" });
    expect(visibleFindings([dismissed], false, "CRITICAL").map((f) => f.id)).toEqual(["d1"]);
  });
});
