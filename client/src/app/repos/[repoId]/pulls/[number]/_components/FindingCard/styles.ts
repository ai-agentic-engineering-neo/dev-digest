...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // All per-side longhand — `borderColor`/`borderWidth` are themselves
    // shorthands covering all four sides, so mixing either with
    // `borderLeftColor`/`borderLeftWidth` still trips React's "conflicting
    // property" warning on rerender. Only fully side-specific props are safe.
    borderStyle: "solid",
    borderTopWidth: 1,
    borderRightWidth: 1,
    borderBottomWidth: 1,
