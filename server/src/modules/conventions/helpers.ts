...
import { MAX_FILE_CHARS, MAX_FILE_LINES, MAX_SNIPPET_LINES, MIN_SNIPPET_CHARS } from './constants.js';
...
function resolveFile(files: Map<string, SampledFile>, cited: string): SampledFile | undefined {
  ...
  const matches = [...files.values()].filter((f) => f.path === clean || f.path.endsWith(`/${clean}`));
 ...
 */
export function buildSkillDraft(repoFullName: string, rows: ConventionRow[]): ConventionSkillDraft {
