...
import { api } from "../api";
import { providerKeys, repoKeys, prKeys, settingsKeys } from "./keys";
import type {
  Settings,
  SettingsUpdate,
  ConnTestProvider,
...
export function useUpdateSettings() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => qc.setQueryData(settingsKeys.current(), data),
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (res) => {
      if (res.ok) {
        ...
        qc.invalidateQueries({ queryKey: settingsKeys.secretsStatus() });
      }
    },
  });
}
...
export function useSecretsStatus() {
  return useQuery({
    queryKey: settingsKeys.secretsStatus(),
...
export function useAddRepo() {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: repoKeys.list() }),
  });
}
...
export function useRefreshRepo() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, repoId) => {
      qc.invalidateQueries({ queryKey: repoKeys.list() });
      qc.invalidateQueries({ queryKey: repoKeys.pulls(repoId) });
    },
  });
}
...
export function useReindexContext() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, repoId) => qc.invalidateQueries({ queryKey: repoKeys.context(repoId) }),
  });
}
