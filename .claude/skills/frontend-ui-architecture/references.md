...
Research compiled 2026-09-21. Focus: architecture and code organization
(where components/constants/utils/business logic live, how components are
split), explicitly **not** performance. Grouped by the question each source
answers in [SKILL.md](SKILL.md).
...
- [Robin Wieruch — React Folder Structure (2026)](https://www.robinwieruch.de/react-folder-structure/) —
  ...
  including the `constants/` vs `utils/` discussion this skill draws on.
...
## 2. Colocation (what lives next to what)

- [Kent C. Dodds — Colocation](https://kentcdodds.com/blog/colocation) —
  the foundational principle this skill's "colocate first, promote later"
  rule is built on: "place code as close to where it's relevant as
  possible"; "things that change together should be located as close as
  reasonable" (attributed to Dan Abramov).
...
- [Next.js docs — Project Structure](https://nextjs.org/docs/app/getting-started/project-structure) —
  official confirmation that files inside `app/` can be safely colocated:
  a route only becomes public once a `page.tsx`/`route.ts` exists in that
  segment.
- [Next.js docs — Routing: Project Organization (colocation)](https://nextjs.org/docs/14/app/building-your-application/routing/colocation) —
  ...
  UI/implementation detail from routing.

## 3. Component splitting (single responsibility, where the boundary is)
...
- [patterns.dev — Container/Presentational Pattern](https://www.patterns.dev/react/presentational-container-pattern/) —
  the classic split between "what data to show" and "how to show it";
  still conceptually useful even though hooks now usually play the
  container's role instead of a wrapper component.
- [Is Atomic Design Still Relevant in 2025?](https://www.designsystemscollective.com/is-atomic-design-still-relevant-in-2025-d9c214788cfe) —
  the critique behind this skill's recommendation to avoid Atomic Design's
  five-tier hierarchy as the primary organizing scheme for product/business
  code: it doesn't map well onto domain logic and fragments a feature
  across unrelated folders. Better suited to an isolated design-system
  package.

## 4. Where business logic lives (hooks vs. pure functions/services)

- [Custom react hooks vs services (DEV.to discussion)](https://dev.to/chiangs/custom-react-hooks-vs-services-mcm) —
  the core distinction this skill relies on: a hook can only be used
  inside the React tree and can hold state/context; a plain function
  (service) can be used anywhere and is trivially unit-testable. Pure
  business logic (validation, calculation, formatting) should be a plain
  function; state/orchestration belongs in a hook.
...
## 5. Constants vs. utils vs. helpers
...
- General consensus synthesized from multiple community write-ups on React
  ...
  paths); utils are pure, side-effect-free, reusable functions; "helper"
  as a term is used inconsistently across teams and most often really
  means "feature-specific util" rather than a distinct category — hence
  this skill's recommendation to fold helpers into a feature's `utils/`
  rather than maintaining a separate, ambiguous `helpers/` folder.
- [TkDodo — Please Stop Using Barrel Files](https://tkdodo.eu/blog/please-stop-using-barrel-files) —
  ...
  circular-dependency bugs; a barrel is only justified as a package's
...
## 7. Architectural boundaries in a larger codebase
...
- [Enforce Architectural Policies in JavaScript with ESLint (DEV.to)](https://dev.to/ptvty/enforce-architectural-policies-in-javascript-with-eslint-mhe) —
  ...
  other directly" from a convention into a lint rule, which is the
  practical way to enforce the boundary rule in this skill's "Top-Level
  Structure" section at scale.
