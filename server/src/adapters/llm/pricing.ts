 ...
 *
 * NOTE: cost sums are null-propagating (`addCost` in reviewer-core): ONE
 * unpriced attempt makes the whole run's `cost_usd` null ("unknown", not a
 * misleading partial sum — there is no column to flag a partial total). So
 * every model the studio can select must be listed here; add new ids when
 * they ship. OpenRouter slugs are priced live by PriceBook first.
 */
interface Price {
  in: number;
  out: number;
...
const PRICING: Record<string, Price> = {
  ...
  // Anthropic — first-party list prices (claude-api skill model table, cached
  // 2026-06-24). Cache/batch discounts are not modelled (upper-bound estimate).
  ...
  // Retired 3.x aliases — kept so historical runs stay priced.
