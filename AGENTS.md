...
This file is intentionally short. Each package has its own `AGENTS.md` (its `CLAUDE.md` is a one-line `@AGENTS.md` import), loaded
automatically when you touch files there. Read the linked docs only when the
task needs them.

## Before answering or changing anything

1. Search the curated docs first: the relevant package's `AGENTS.md`, `README.md`, `docs/`, `specs/`, and `INSIGHTS.md`, plus the root `docs/`. They may already answer the question or record a decision.
...
|---|---|---|---|
| `server/` | `@devdigest/api` | Fastify 5 + Drizzle + Postgres (pgvector), `:3001` | `server/AGENTS.md` |
| `client/` | `@devdigest/web` | Next.js 15 studio UI, `:3000` | `client/AGENTS.md` |
| `reviewer-core/` | `@devdigest/reviewer-core` | Pure review engine: diff → prompt → LLM → grounded findings | `reviewer-core/AGENTS.md` |
| `e2e/` | `@devdigest/e2e` | Deterministic browser flows (agent-browser) | `e2e/AGENTS.md` |
| `server/src/vendor/shared/` | `@devdigest/shared` | Zod contracts + adapter interfaces (canonical copy) | see `server/AGENTS.md` |
