      ...
      <div style={s.page}>
        ...
        <div style={s.headerRow}>
          ...
          <Button icon="RefreshCw" onClick={runScan} loading={extract.isPending} disabled={extract.isPending}>
            {extract.isPending ? t("page.scanning") : scanned ? t("page.rescan") : t("page.runExtraction")}
          </Button>
        </div>
            ...
            {counts.accepted > 0 && (
              <>
                <Button
                  ...
                  size="sm"
                  icon="X"
                  onClick={() => setExcluded(excluded.size === 0 ? new Set(acceptedIds) : new Set())}
                >
                  {excluded.size === 0 ? t("page.deselectAll") : t("page.selectAll")}
                </Button>
                <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                  {t("page.selectedCount", { selected: selectedIds.length, total: counts.accepted })}
                </span>
              </>
            )}
            <Button
        ...
        {!isLoading && !isError && scanned && visible.length === 0 && (
          <EmptyState icon="ListChecks" title={t("page.emptyFiltered.title")} body={t("page.emptyFiltered.body")} />
        )}
